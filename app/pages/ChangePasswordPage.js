...
import { updateProfile, updatePassword, EmailAuthProvider } from "firebase/auth";
...
export function ChangePasswordPage({ navigation }) {
  ...
  const handleOnSubmit = () => {
    (async () => {
      ...
      isSuccessful ? 
        (() => { Toast.show("Password updated sucessfully."); changePasswordPageOperations.resetData(); })() : 
      ...
      <View style={{ position: "relative", display: "flex", alignItems: "center", width: "100%", height: "100%", paddingTop: 30 }}>
        ...
        <Card>
          <View style={{ display: "flex", alignItems: "center", width: "100%" }}>
            <PasswordTextBox placeholder={"Enter Old Password"} value={changePasswordPageOperations.oldPwdState[0]} onChange={(oldPwd) => changePasswordPageOperations.oldPwdState[1](oldPwd)} />
            <PasswordTextBox placeholder={"Enter New Password"} value={changePasswordPageOperations.newPwdState[0]} onChange={(newPwd) => changePasswordPageOperations.newPwdState[1](newPwd)} />
          </View>
          <PrimaryButton text={"Confirm"} onPress={() => handleOnSubmit()} isDisabled={changePasswordPageOperations.isNewPwdInvalid()} />
        </Card>
      </View>
...
function Card({ children }) {
  return (
    <View style={{ display: "flex", alignItems: "center", justifyContent: "space-evenly", width: "90%", height: vh(45), backgroundColor: "white", borderWidth: 0, borderRadius: 20,
      elevation: 2,
      shadowColor: "black",
...
class ChangePasswordPageOperations {
  ...
  async updatePassword() {
    ...
    try {
      await this.user.reauthenticateWithCredential(EmailAuthProvider.credential(this.user.email, this.oldPwdState[0]));
    } catch {
      return false;
    }
