...
import { TouchableWithoutFeedback, Keyboard, Text, View, Button, TouchableOpacity, TextInput, Image } from "react-native";
import { colors } from "../styling/appTheme";
import { vh } from "react-native-expo-viewport-units";
import firebaseApp, { auth, storage, storagePaths } from "../../src/firebase/config";
...
export function EditProfilePageHeader(props) {
  return (
    <View style={{ position: "relative", display: "flex", flexDirection: "row", alignItems: "center", paddingTop: 50, paddingLeft: 20, backgroundColor: colors.secondary }}>
      ...
      <Text style={{ color: "white", fontSize: 22, fontWeight: 'bold' }}>{auth.currentUser?.displayName ?? "User"}</Text>
    </View>
  )
}

export function EditProfilePage({ navigation }) {
  const editProfilePageOperations = new EditProfilePageOperations(auth.currentUser, getProfileImageRef);
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <View style={{ position: "relative", display: "flex", alignItems: "center", width: "100%", height: "100%", paddingTop: 30 }}>
        <PurpleBackdrop />
        <Card>
          <EditProfilePhotoSection uri={editProfilePageOperations.profileImageUriState[0]} onPress={async () => {
          ...
          <View style={{ display: "flex", alignItems: "center" , width: "100%" }}>
            <TextBox placeholder="First Name" value={editProfilePageOperations.firstNameState[0]} onChange={(fName) => editProfilePageOperations.firstNameState[1](fName)} />
            <TextBox placeholder="Last Name" value={editProfilePageOperations.lastNameState[0]} onChange={(lName) => editProfilePageOperations.lastNameState[1](lName)} />
            <TextBox placeholder="Email" value={editProfilePageOperations.emailState[0]} onChange={(email) => editProfilePageOperations.emailState[1](email)} />
          </View>
...
function Card({ children }) {
  return (
    <View style={{ display: "flex", alignItems: "center", justifyContent: "space-evenly", width: "90%", minHeight: vh(65), backgroundColor: "white", borderWidth: 0, borderRadius: 20,
      elevation: 2,
      shadowColor: "black",
  ...
  return (
    <View style={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "center", width: "100%" }}>
      ...
      <TouchableOpacity activeOpacity='0.5'
        style={{ backgroundColor: colors.tertiary, paddingTop: 10, paddingRight: 20, paddingBottom: 10, paddingLeft: 20, borderRadius: 10, marginLeft: 20 }}
...
function PrimaryButton({ text, isDisabled, onPress }) {
  return (
    <TouchableOpacity activeOpacity='0.5'
      ...
      style={{ width: "60%", backgroundColor: isDisabled ? "lightgrey" : colors.tertiary, paddingTop: 10, paddingRight: 20, paddingBottom: 10, paddingLeft: 20, borderRadius: 10 }}
    >
...
class EditProfilePageOperations {
  ...
  async updateDisplayName() {
    ...
    try {
      await updateProfile(this.user, { displayName: `${this.firstNameState[0]} ${this.lastNameState[0]}` });
      return true
    } catch {
      return false;
    }
  ...
  async updateEmail() {
    if (this.initialEmail === this.emailState[0]) 
      return true;
